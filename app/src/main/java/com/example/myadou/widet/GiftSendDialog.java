...
import com.example.myadou.widet.gift.GiftGridView;

import java.util.ArrayList;

/**
 * Created by 张晓辉 on 2018/1/29.
 * 送礼物的Dialog
 */
...
public class GiftSendDialog implements View.OnClickListener {
    ...
    //提供接口的实例变量
    private OnGiftSendListenter giftSendListenter;

    public GiftSendDialog(@NonNull Activity activity,OnGiftSendListenter onGiftSendListenter) {
        this.activity = activity;
        dialog = new Dialog(activity);
        giftSendListenter=onGiftSendListenter;
        init();
        initListener();
    }
        ...
        bt_send_gift.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if (System.currentTimeMillis()-preClickTime>50){
                    //让调用者去处理发送按钮的点击事件
                    if (giftSendListenter!=null){
                        giftSendListenter.onSend(selectedGift);
                    }
                }else {
                    ToastUtils.show("点击的太快了");
                }
    ...
    public GiftSendDialog(@NonNull Activity activity, int themeResId,OnGiftSendListenter onGiftSendListenter) {
        this.activity = activity;
        //把dialog实例化
    ...
    //去让调用者去处理点击事件
    public interface  OnGiftSendListenter{
